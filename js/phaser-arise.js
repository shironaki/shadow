'use strict';
/* Shadow Ascension · Arise presentation v0.2 */
(function(){
 ...
 function silhouette(scene,p){const c=scene.add.container(p.x,p.y+8);const g=scene.add.graphics();g.fillStyle(0x09070f,.96);g.fillEllipse(0,4,30,9);g.fillEllipse(0,-10,15,22);g.fillTriangle(0,-30,-11,0,11,0);g.fillStyle(0xc4b5fd,1);g.fillCircle(-2,-12,1.5);g.fillCircle(2,-12,1.5);c.add(g);c.setDepth(p.y+300);c.setScale(.08);c.setAlpha(0);scene.tweens.add({targets:c,scale:.9,alpha:1,y:p.y-34,duration:500,ease:'Cubic.easeOut'});scene.tweens.add({targets:c,alpha:0,scale:.55,duration:260,delay:520,onComplete:()=>c.destroy()});return c}
 function label(scene,p){const t=scene.add.text(p.x,p.y-52,'АРИЗ!',{fontFamily:'monospace',fontSize:'15px',fontStyle:'bold',stroke:'#09070f',strokeThickness:4});t.setOrigin(.5);t.setDepth(p.y+500);scene.tweens.add({targets:t,y:p.y-78,alpha:0,duration:700,delay:120,onComplete:()=>t.destroy()})}
 function tick(){const apex=globalThis.__shadowPhaserApex,scene=apex&&apex.scene,G=get('G');if(!scene||!G){requestAnimationFrame(tick);return}const s=apex.stats||scene,now=performance.now(),dt=Math.min(.05,(now-last)/1000);last=now;const shadows=arr(G.shadows),corpses=arr(G.corpses),current=new Set;
  ...
  /* If the simulation removes corpses immediately after X, the shadow-array delta is still enough to trigger Arise. */
  corpses.forEach((c,i)=>{if(!c)return;const id='c:'+String(c.uid??c.id??i);if(fx.has(id))return;fx.add(id);const p=iso(num(c.x),num(c.y),s);burst(scene,p);});
  /* Keep the known set bounded when an army is replaced/reset. */
