...
(function(){
 ...
 function tick(now){const apex=globalThis.__shadowPhaserApex,scene=apex&&apex.scene,G=get('G');if(!scene||!G){requestAnimationFrame(tick);return}if(!root){root=scene.add.container(0,0);root.setDepth(950000)}const s=apex.stats||scene,dt=Math.min(.05,(now-last)/1000);
