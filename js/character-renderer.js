...
class CharacterRenderer {
  constructor(ctx, manifest, characterId, options = {}) {
    ...
    this.frameW=manifest.canvas.logicalWidth; this.frameH=manifest.canvas.logicalHeight; this.state='idle'; this.frame=0; this.elapsed=0; this.done=false; this.flipX=false;
  ...
  draw(options={}){const img=this.images[this.state];if(!img)return false;const c=this.ctx,s=options.scale??this.scale,x=options.x??this.x,y=options.y??this.y,a=options.alpha??1,dw=this.frameW*s,dh=this.frameH*s,dx=x-dw/2,dy=y-dh*this.anchorY,sx=Math.min(this.frame,img.width/this.frameW-1)*this.frameW;c.save();c.globalAlpha=a;c.imageSmoothingEnabled=false;if(this.shadow){c.save();c.globalAlpha=.22*a;c.fillStyle='#000';c.beginPath();c.ellipse(x,y+5*s,28*s,7*s,0,0,Math.PI*2);c.fill();c.restore()}c.translate(dx+(this.flipX?dw:0),dy);if(this.flipX)c.scale(-1,1);c.drawImage(img,sx,0,this.frameW,this.frameH,0,0,dw,dh);c.restore();return true}
}
