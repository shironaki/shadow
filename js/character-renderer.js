...
class CharacterRenderer {
  constructor(ctx, manifest, characterId, options = {}) {
    ...
    this.pack=manifest.spritePacking||{}; this.columns=Math.max(1,this.pack.columnsPerFrame||3); this.characterColumn=Math.min(this.columns-1,Math.max(0,this.pack.characterColumn??1));
...
(function(){
  ...
  let manifest=null,renderer=null,loadingId='',loadToken=0,drawLocked=false;
  ...
  window.__shadowCharacterDraw=function(X,Y){if(drawLocked)return;const g=game();if(renderer&&!renderer.draw({x:X,y:Y,scale:.34,alpha:g&&g.stealth>0?.4:.98}))return;drawLocked=true};
  globalThis.eval('drawPlayer=window.__shadowCharacterDraw');
  function installUpdateHook(){let legacyUpdate;try{legacyUpdate=globalThis.eval('update')}catch(_){return false}window.__shadowCharacterUpdate=function(dt){if(typeof legacyUpdate==='function')legacyUpdate(dt);sync();if(renderer)renderer.update(dt*1000);drawLocked=false};globalThis.eval('update=window.__shadowCharacterUpdate');return true}
