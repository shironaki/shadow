...
(function(){
  ...
  window.__shadowCharacterDraw=function(X,Y){if(drawLocked)return;const g=game();if(renderer&&!renderer.draw({x:X,y:Y,scale:.34,alpha:g&&g.stealth>0?.4:.98})){drawLocked=true;return}if(renderer){drawLocked=true;return}drawLocked=true};
  globalThis.eval('drawPlayer=window.__shadowCharacterDraw');
  function installUpdateHook(){let legacyUpdate;try{legacyUpdate=globalThis.eval('update')}catch(_){return false}if(window.__shadowCharacterUpdateInstalled)return true;window.__shadowCharacterUpdateInstalled=true;window.__shadowCharacterUpdate=function(dt){if(typeof legacyUpdate==='function')legacyUpdate(dt);sync();if(renderer)renderer.update(dt*1000);drawLocked=false};globalThis.eval('update=window.__shadowCharacterUpdate');return true}
