...
const ET={
 ...
 baran:{n:'Баран, Владыка Белого Пламени',hp:2000,atk:34,spd:1.9,r:.7,exp:1100,ag:13,rng:6.5,keep:3.4,cd:1.7,ranged:1,burst:1,boss:1,spec:'lightning',g:[240,480],rise:0},
...
const CLASSES={ // v0.10: выбор класса при создании героя
 shade:{n:'Тень',d:'Ассасин: +6% крит, скрытность дольше. Классический стиль.',icon:'dagger'},
 ward:{n:'Страж',d:'Живучесть: +22% здоровья, −18% получаемого урона, чуть медленнее.',icon:'shield'},
 mage:{n:'Чародей',d:'Магия: +25% маны, навыки дешевле на 25%, мана быстрее.',icon:'gem'}};
...
const REL={kasaka:{n:'Клинок Гадюки',desc:'Ваши удары парализуют врагов (18%, 0.7с). Не действует на владык.'},baruka:{n:'Кинжал Вихря',desc:'+12% к шансу крита, критический урон ×3.4.'},monolith:{n:'Слеза Монолита',desc:'Вне боя восстанавливает 1.5% здоровья каждые 5 секунд.'},heart:{n:'Сердце Владыки',desc:'+50% к набору ярости.'}};
...
/* ПРЕДМЕТЫ */
const WNAMES=[['Клинок Гадюки','Теневой клинок','Кинжал Вихря','Клинки Владыки'],['Кожаный доспех','Доспех стража','Латы рыцаря','Броня Владыки Теней'],['Медное кольцо','Кольцо теней','Печать мага','Печать Владыки']];
...
function calcStats(){
 ...
 // v0.10: классовые особенности
 const CL=p.cls||'shade';
 if(CL==='ward'){p.maxhp=Math.round(p.maxhp*1.22);p.atk=Math.round(p.atk*.92);p.spd*=.92}
 if(CL==='mage'){p.maxmp=Math.round(p.maxmp*1.25);p.mpRegen*=1.6}
 if(CL==='shade')p.crit=Math.min(75,p.crit+6);
